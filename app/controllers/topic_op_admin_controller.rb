...
class TopicOpAdminController < ::ApplicationController
  ...
  def update_topic_status
    ...
    generate_with_perm_logger_text =
      begin
        ...
        "@#{guardian.user.username} " +
          I18n.t("topic_op_admin.log_template.with_perm.#{status}.#{enable_text}").gsub("#", topic.url) +
    ...
    generate_without_perm_logger_text =
      begin
        enable_text = enabled ? ".enable" : ".disable"
        "@#{guardian.user.username} " +
          I18n.t("topic_op_admin.log_template.without_perm.#{status}.#{enable_text}").gsub("#", topic.url) +
          "\n#{I18n.t("topic_op_admin.log_template.reason")} #{params[:reason]}"
      end
    ...
    render json:
             success_json.merge!(
               topic_status_update: TopicTimerSerializer.new(TopicTimer.find_by(topic: topic), root: false),
             )
  end
  ...
  def update_slow_mode
    ...
    unless guardian.can_set_topic_slowmode_as_op?(topic)
      TopicOpUserAdminBot.botLogger(
        "@#{guardian.user.username} " +
          I18n.t("topic_op_admin.log_template.without_perm.slow_mode." + (enabled ? "enable" : "disable")).gsub(
            "#",
            topic.url,
          ) + "\n```\n#{params.to_yaml}\n```",
      )
    ...
    TopicOpUserAdminBot.botLogger(
      "@#{guardian.user.username} " +
        I18n.t("topic_op_admin.log_template.with_perm.slow_mode." + (enabled ? "enable" : "disable")).gsub(
          "#",
          topic.url,
        ) + "\n```\n#{params.to_yaml}\n```",
    )
  ...
  def request_for_topic_op_admin
    ...
    post =
      PostCreator.create!(
        ...
        target_group_names: SiteSetting.topic_op_admin_manipulatable_groups_map.map { |id| Group.find_by(id:).name },
    ...
    if topic.topic_op_admin_status?.is_default
      ...
      if SiteSetting.topic_op_admin_basic_autoallowing_tags.to_s.split("|").intersect?(topic.tags.map(&:name)) ||
           SiteSetting
      ...
      %w[can_close can_visible can_make_PM can_archive can_set_timer can_silence can_slow_mode].each do |status|
        if SiteSetting
  ...
  def set_topic_op_timer
    ...
    if !guardian.can_set_topic_timer_as_op?(topic) || TopicTimer.destructive_types.values.include?(status_type)
      TopicOpUserAdminBot.botLogger(
        "@#{guardian.user.username} " +
    ...
    TopicOpUserAdminBot.botLogger(
      "@#{guardian.user.username} " + I18n.t("topic_op_admin.log_template.with_perm.set_timer").gsub("#", topic.url) +
        "\n```\n#{params.to_yaml}\n```",
    )
  ...
  def update_topic_op_banned_users
    ...
    return render_fail "topic_op_admin.too_many", status: 403 if (params[:new_silence_users].length > 100)
    ...
    TopicOpUserAdminBot.botLogger(
      "@#{guardian.user.username} " + I18n.t("topic_op_admin.log_template.with_perm.silence").gsub("#", topic.url) +
        "\n```\n#{params.to_yaml}\n```",
    )
    ...
    unmute_line = ""
    unmute_line = I18n.t("topic_op_admin.bot_send_template.unmute.text") + ": " + unmute_names + "\n" if unmute_names !=
      ""
    ...
    if !(mute_line == "" && unmute_line == "")
      TopicOpUserAdminBot.botSendPost(
        topic.id,
        "@#{guardian.user.username}\n\n" + unmute_line + mute_line + I18n.t("topic_op_admin.log_template.reason") +
          " #{params[:reason]}",
        skip_validations: true,
      )
    end
